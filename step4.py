...
_data_airp = pd.read_csv('data/energy_avg_.csv', usecols=[2, 8], names=['valid_datetime', 'GSolar_MW'], skiprows=1)
_data_airp['Time'] = pd.to_datetime(_data_airp['valid_datetime'], errors='raise')
_trainp = 80
idx_split = round(len(_data_airp['GSolar_MW']) * (_trainp/100)) # Split train-test
_datatrain = _data_airp.iloc[:idx_split+1]
_opts = range(0,int(len(_datatrain['GSolar_MW'])/2),1)
_opts = list(_opts)
...
def predict_(_p, _d, _q, _P, _D, _Q, _m, _noffs, _ntype):
    ...
    idx_split = round(len(_data['GSolar_MW']) * (_trainp/100)) # Split train-test # Split train-test
    ...
    _data_pred['Time'] = tt
    _data_pred['GSolar_MW'] = np.nan
    _data_pred['_is_train'] = 2
    _data_all = pd.concat([_data_train, _data_test, _data_pred], ignore_index=True)

    # Fit model
    _best_model = SARIMAX(endog = _data_train['GSolar_MW'], order=(_p, _d, _q), seasonal_order=(_P, _D, _Q, _m)).fit(disp=-1)
    ...
    _data_all['Values'] = np.exp(_data_all['GSolar_MW'])
    ...
    _data_all.loc[_data_all['Values Predicted'] <= min(list(_data_all['GSolar_MW']))/2, 'Values Predicted'] = np.nan # Correcting outliers
    _data_all.loc[_data_all['Values Predicted'] >= max(list(_data_all['GSolar_MW']))*2, 'Values Predicted'] = np.nan
    _data_all.loc[_data_all['Pred CI lower'] <= min(list(_data_all['GSolar_MW']))/2, 'Pred CI lower'] = np.nan
    _data_all.loc[_data_all['Pred CI upper'] >= max(list(_data_all['GSolar_MW']))*2, 'Pred CI upper'] = np.nan
    ...
    fig1.add_trace(go.Scatter(x=_data_all.loc[_data_all['_is_train']==1, 'Time'],
                                y=_data_all.loc[_data_all['_is_train']==1, 'GSolar_MW'], mode='lines', name='Train', line=train_linelayout))
    fig1.add_trace(go.Scatter(x=_data_all.loc[_data_all['_is_train']==0, 'Time'],
                                y=_data_all.loc[_data_all['_is_train']==0, 'GSolar_MW'], mode='lines', name='Test', line=test_linelayout))
    ...
    fig1.update_yaxes(title_text = 'GSolar_MW')
